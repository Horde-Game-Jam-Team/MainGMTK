...
void ARPGCharacter::InitializeAttributes()
{
	if (AbilitySystemComponent && AttributeSet)
	{
		...
		//Bind Resources Change event
		AbilitySystemComponent->GetGameplayAttributeValueChangeDelegate(AttributeSet->GetCurrentManaAttribute()).AddUObject(this, &ARPGCharacter::HandleManaChanged);
		AbilitySystemComponent->GetGameplayAttributeValueChangeDelegate(AttributeSet->GetCurrentStaminaAttribute()).AddUObject(this, &ARPGCharacter::HandleStaminaChanged);
		AbilitySystemComponent->GetGameplayAttributeValueChangeDelegate(AttributeSet->GetFireCrystalsAttribute()).AddUObject(this, &ARPGCharacter::HandleFireCrystalsChanged);


		//Bind to Movement speed change event
...
// Functions to handle Resource changes and update UI
void ARPGCharacter::HandleManaChanged(const FOnAttributeChangeData& Data)
{
	// Handle resource change logic here, such as updating UI or triggering events
	float NewResource = Data.NewValue;
	float OldResource = Data.OldValue;
	float DeltaResource = NewResource - OldResource;

	// Call the Blueprint function to update UI
	OnManaChanged(DeltaResource, FGameplayTagContainer());
}

void ARPGCharacter::HandleStaminaChanged(const FOnAttributeChangeData& Data)
{
	// Handle resource change logic here, such as updating UI or triggering events
	float NewResource = Data.NewValue;
	float OldResource = Data.OldValue;
	...
	// Call the Blueprint function to update UI
	OnStaminaChanged(DeltaResource, FGameplayTagContainer());
}

void ARPGCharacter::HandleFireCrystalsChanged(const FOnAttributeChangeData& Data)
{
	// Handle resource change logic here, such as updating UI or triggering events
	float NewResource = Data.NewValue;
	float OldResource = Data.OldValue;
	...
	// Call the Blueprint function to update UI
	OnFireCrystalsChanged(DeltaResource, FGameplayTagContainer());
}
